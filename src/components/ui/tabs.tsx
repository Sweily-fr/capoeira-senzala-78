...
const tabsListVariants = cva('shrink-0', {
  variants: {
    orientation: {
      ...
      vertical: 'flex-col items-start space-y-2',
    },
    variant: {
...
const Tabs = React.forwardRef<
  ...
  TabsProps
>(({ className, orientation = 'horizontal', variant = 'default', size = 'md', children, ...props }, ref) => {
...
>(({ 
  ...
  const { variant: contextVariant, size: contextSize, orientation: contextOrientation } = React.useContext(TabsContext);
  ...
  return (
    <TabsPrimitive.List
      ...
      className={cn(
        'flex',
        tabsListVariants({ variant, shape, size, orientation }),
        className
      )}
      {...props}
    />
